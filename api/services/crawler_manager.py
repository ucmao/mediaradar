...
class CrawlerManager:
    ...
    def __init__(self):
        ...
        self._baseline_fingerprints: dict[tuple[str, str, str], str] = {}
    ...
    async def start(self, config: CrawlerStartRequest) -> bool:
        ...
        async with self._lock:
            ...
            self._baseline_fingerprints = analytics_repository.content_fingerprints(
                load_normalized_contents_from_sqlite(analytics_repository.db_path, config.platform.value)
            )
    ...
    async def stop(self) -> bool:
        ...
        async with self._lock:
            ...
            await self._finalize_run("stopped", exit_code)
            self.status = "idle"
    ...
    def _collect_run_contents(self, config: CrawlerStartRequest) -> list[dict]:
        """Collect records created or updated during the active crawler run."""
        contents = load_normalized_contents_from_sqlite(analytics_repository.db_path, config.platform.value)
        ...
        changed_keys = {
            ...
            if self._baseline_fingerprints.get(key) != fingerprint
        }
        platform = config.platform.value
        keywords = {value.strip() for value in config.keywords.split(",") if value.strip()}
        return [
            item for item in contents
            if (item["platform"], item["content_id"], item["keyword"]) in changed_keys
            and item["platform"] == platform
            and (not keywords or item["keyword"] in keywords or config.crawler_type.value != "search")
        ]

    async def _finalize_run(self, status: str, exit_code: int | None, error_message: str = "") -> None:
        """Persist run output exactly once, including stopped and failed runs."""
        async with self._finalize_lock:
            ...
            try:
                contents = await asyncio.to_thread(self._collect_run_contents, config) if config else []
                ...
                entry = self._create_log_entry(f"Failed to save analytics: {error}", "error")
                await self._push_log(entry)
            finally:
                self.last_run_id = run_id
                self.current_run_id = None
        ...
        try:
            ...
            if self.status == "running":
                ...
                if exit_code == 0:
                    ...
                    run_status = "failed"
                await self._push_log(entry)
                await self._finalize_run(run_status, exit_code)
                self.status = "idle"
            ...
            await self._push_log(entry)
            await self._finalize_run("failed", self.process.returncode if self.process else None, str(e))
            self.status = "error"
