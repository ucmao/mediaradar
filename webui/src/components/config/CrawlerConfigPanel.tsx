...
function Section({ title, description, icon: Icon, children, className = '', compact = false }: SectionProps) {
  return (
    <section className={`rounded-lg glass-panel float-panel overflow-hidden ${className}`}>
      <header className={`${compact ? 'px-3 py-2 gap-2' : 'px-4 py-3 gap-3'} border-b border-cyber-border-subtle/50 flex items-center bg-cyber-bg-tertiary/30`}>
        <div className={`${compact ? 'h-7 w-7' : 'h-8 w-8'} rounded-md bg-cyber-bg-tertiary border border-cyber-border-subtle flex items-center justify-center flex-shrink-0`}>
...
type KeywordInputHandle = {
  commitPending: () => string
}

const KeywordInput = forwardRef<KeywordInputHandle, KeywordInputProps>(function KeywordInput(
  ...
  const commitPending = () => {
    ...
    const nextValue = keywords.includes(trimmed)
      ? value
...
export function CrawlerConfigPanel() {
  ...
  const keywordInputRef = useRef<KeywordInputHandle>(null)
  ...
  const handleStart = () => {
    const keywords = config.crawler_type === 'search'
      ? keywordInputRef.current?.commitPending() ?? config.keywords
      : config.keywords
  ...
  return (
    <div className="h-full min-h-0 flex flex-col gap-4 animate-slide-up">
