...
export function Terminal() {
  ...
  const handleStartSingle = (e: MouseEvent, p: string) => {
    e.stopPropagation()
    startPlatform({ ...config, platform: p })
  }
          ...
          <div className="flex items-center gap-3 text-[11px] font-mono">
            ...
            {activeStatus === 'running' && (
              <div className="flex items-center gap-1">
                ...
                <span className="text-cyber-neon-green font-bold text-[10px] uppercase">Active</span>
              </div>
            )}
          </div>
...
{`  ╔═════════════════════════════════════════════════════════╗
  ...
  ║ | |\\/| |/ _ \\/ _\` | |/ _\` | |_) / _\` |/ _\` |/ _\` | '__| ║
  ║ | |  | |  __/ (_| | | (_| |  _ < (_| | (_| | (_| | |    ║
  ║ |_|  |_|\\___|\\__,_|_|\\__,_|_| \\_\\__,_|\\__,_|\\__,_|_|    ║
  ║                                                         ║
  ║             [ NEURAL SCANNING TAB: ${activePlatformTab.toUpperCase()} ]             ║
  ╚═════════════════════════════════════════════════════════╝`}
                </pre>
                <div className="text-[#c9d1d9] text-[11px] space-y-1">
                  <p className="text-cyber-neon-green/70">
                    &gt;_ Terminal session ready. Awaiting instructions...
                  </p>
                  <p className="text-[#8b949e]">
                    &gt;_ Select targets and click 'Initiate Scan' to start harvesting intelligence.
                  </p>
                </div>
              </div>
            ) : (
          ...
          <div className="px-4 py-1.5 border-t border-[#30363d] bg-[#161b22] flex items-center justify-between flex-shrink-0 select-none">
            <span className="text-[10px] font-mono text-[#8b949e]">
              LOOP MODE: {config.loop_execution ? 'ENABLED' : 'DISABLED'}
            </span>
